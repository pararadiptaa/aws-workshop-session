...
import {
    ...
    Search, ShieldCheck, Mail, RefreshCw, Star, Shirt, ChefHat
} from 'lucide-react';
            ...
            <div className="flex gap-4 justify-center">
                {tabs.map((t) => (
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-full transition-all border ${activeTab === t.id
...
export const Storage = () => {
    ...
    const data = {
        ebs: {
            label: "EBS (Block)",
            analogyTitle: "The Walk-In Closet",
            theory: "Raw storage volumes attached to a single server. Extremely low latency (fast), dedicated to one instance, supports granular editing (change one byte without rewriting the file).",
            analogy: "Block Storage is like the Walk-In Closet inside your Bedroom. It is directly attached to your room (Server). It is private—only you can touch the clothes. You can swap just your socks (granular edit) without having to replace your entire wardrobe.",
            icon: DoorOpen
        },
        efs: {
            label: "EFS (File)",
            analogyTitle: "The Kitchen Pantry",
            theory: "Hierarchical storage (Folders/Files) supporting standard network protocols (NFS). Shared access. Multiple servers can read/write simultaneously.",
            analogy: "File Storage is like the Shared Kitchen Pantry. It has shelves (Folders) and jars (Files). Unlike the private closet, this is for sharing. Multiple family members (Servers) can walk in, grab ingredients, or add new groceries at the same time.",
            icon: Utensils
        },
        s3: {
            label: "S3 (Object)",
            analogyTitle: "Amazon Fulfillment Center",
            theory: "Flat storage managed as 'Objects' (Data + Metadata + ID). Infinite scalability, accessed via API. Immutable (must replace the whole object to change it).",
            analogy: "Object Storage is like a massive Amazon Fulfillment Center. You don't walk in; you send a request with an Order ID (Key). It is infinite and cheap, but if you want to change a product, you can't just 'edit' it on the shelf. You must return the box and send a completely new one.",
            icon: Warehouse
        }
    };
                        ...
                        {activeTab === 'ebs' && (
                            ...
                            >
                                <div className="flex items-center justify-center gap-0 relative">
                                    {/* The Bedroom/Server */}
                                    <div className="border-2 border-depth w-24 h-32 flex flex-col items-center justify-end pb-2 rounded-l-lg bg-black relative">
                                        <div className="text-[8px] text-depth absolute top-2">Bedroom</div>
                                        <User size={24} className="text-white mb-2" />
                                    </div>

                                    {/* The Closet */}
                                    <div className="border-2 border-accent w-16 h-32 flex flex-col items-center justify-center rounded-r-lg bg-accent/10 border-l-0 relative overflow-hidden">
                                        <div className="text-[8px] text-accent absolute top-2">Closet</div>
                                        <div className="flex flex-col gap-1">
                                            {/* Socks */}
                                            <motion.div
                                                animate={{ backgroundColor: ["#ffffff", "#9702EA", "#ffffff"] }}
                                ...
                                <div className="text-[10px] text-accent">Granular Edit: Swapping Socks</div>
                            </motion.div>
                        )}
                        {activeTab === 'efs' && (
                            <motion.div
                            ...
                            >
                                {/* Pantry Shelves */}
                                <div className="w-32 h-40 border-2 border-blue-500 rounded bg-blue-500/10 flex flex-col justify-evenly p-2 relative z-10">
                                    <div className="absolute top-1 left-1/2 -translate-x-1/2 text-[8px] text-blue-500 font-bold">PANTRY</div>
                                    <div className="w-full h-1 bg-blue-500/50 rounded" />
                                    <div className="w-full h-1 bg-blue-500/50 rounded" />
                                    <div className="w-full h-1 bg-blue-500/50 rounded" />

                                    <div className="absolute top-6 left-4 w-4 h-4 bg-white rounded-full flex items-center justify-center"><div className="w-2 h-2 bg-blue-500 rounded-full" /></div>
                                    <div className="absolute top-20 right-4 w-4 h-4 bg-white rounded-full flex items-center justify-center"><div className="w-2 h-2 bg-blue-500 rounded-full" /></div>
                                </div>

                                {/* Users Accessing */}
                                <motion.div animate={{ x: [0, 10, 0] }} transition={{ duration: 2, repeat: Infinity }} className="absolute left-8 bottom-8 flex flex-col items-center">
                                    <User size={24} className="text-white" />
                                    <div className="w-16 h-0.5 bg-white/30 rotate-[-45deg] absolute top-[-20px] left-4" />
                                </motion.div>
                                <motion.div animate={{ x: [0, -10, 0] }} transition={{ duration: 2.5, repeat: Infinity }} className="absolute right-8 bottom-8 flex flex-col items-center">
                                    <User size={24} className="text-white" />
                                    <div className="w-16 h-0.5 bg-white/30 rotate-[45deg] absolute top-[-20px] right-4" />
                                </motion.div>

                                <div className="absolute bottom-2 text-[10px] text-depth">Shared Access</div>
                            </motion.div>
                        )}
                        {activeTab === 's3' && (
                            <motion.div
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    <div className="flex flex-col items-center">
                                        ...
                                        <span className="text-[8px] text-depth">Order ID</span>
                                    </div>
                                    <ArrowRight className="text-accent animate-pulse" />
                                    <div className="flex flex-col items-center p-2 border-2 border-depth rounded bg-secondaryDepth/20 relative">
                                        <Warehouse size={48} className="text-depth" />
                                        <span className="text-[8px] text-depth mt-1">Fulfillment Center</span>
                                    </div>
                                </div>

                                <div className="flex items-center gap-2 bg-red-500/10 p-2 rounded border border-red-500/30">
                                    <Box size={16} className="text-red-500" />
                                    <span className="text-[10px] text-red-500">Immutable: Replace Box, Don't Edit</span>
                                </div>
                            </motion.div>
                        )}
                    </AnimatePresence>
